...
package com.example.infoquakes.ui.timeline;
...
import com.example.infoquakes.ui.MainActivity;
import com.example.infoquakes.R;
import com.example.infoquakes.helpers.adapters.RssFeedListAdapter;
...
import static com.example.infoquakes.ui.MainActivity.backFlag;


//this is the class where list of all earthquake data is shown in list
...
public class TimelineFragment extends Fragment{
    ...
    //here the menu is inflated to top right icon to filter result
    @Override
    ...
    //here when user select an option from 3 (show all,filter by date,filter by location).
    //onclick implementation is done here in this function
    @Override
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.all_item:
                ...
                return true;
            //when user click filer by date,dialog box is shown to select the date
            //Calender builtin class of android is used
            //Datepicker builtin class is used to pick a date
            //when date is picked then the date is passed to getfilter() to filter the result based on a date
            // getfilter() is implemented in RssFeedlistAdapter class
            case R.id.bydate_item:
                ...
                return true;
            //same as filter by location
            //spinner is used that shows dropdown list and user select a location
            //getfilter is used to filter the results
            case R.id.bylocation_item:

                new FetchFeedTask().execute((Void) null);
                final AlertDialog.Builder mydialog=new AlertDialog.Builder(getContext());
                LayoutInflater inflater=LayoutInflater.from(getContext());
                View myview=inflater.inflate(R.layout.custom_filter_loc,null);
                mydialog.setView(myview);
                final AlertDialog dialog=mydialog.create();
                dialog.show();

                spinnerLoc=myview.findViewById(R.id.loc_spinner);

                    spinnerLoc.setAdapter(dataAdapter);


                Button filterbtn=myview.findViewById(R.id.filterbtn);
                filterbtn.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        rssFeedListAdapter.getFilter().filter(spinnerLoc.getSelectedItem().toString());
                        dialog.dismiss();
                    }
                });
...
//Pull parser constructor
    ...
    private class FetchFeedTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected void onPreExecute() {
            ...
            urlLink = "http://www.quakes.bgs.ac.uk/feeds/WorldSeismology.xml";
        }
...
//                return false;
        ...
        protected void onPostExecute(Boolean success) {
            ...
            //here is code of when user click a list item
             //the data is passed to google maps to show it
             //the data is also stored in list name(mFeedModelList) and it is set as adapter in spinner to filter by location option
                // Fill RecyclerView
    ...
    //class used for timeline fragment list item click view back button to back timeline fragment
